...
public class ServiciosdeBus {
	...
	public void deshabilitaProductos(Map<String, Object> map) {
		...
		List<Items> productosSinStock = (List<Items>) map.get(PRODUCTOS_SIN_STOCK_KEY);
		Long restaurantID = (Long) map.get("restaurantID");

		if (productosSinStock != null && productosSinStock.size() > 0) {
			for (Items item : productosSinStock) {
				Long codigoProducto = item.getSku();
				...
				try {
					...
					//apiClient.getMenusClient().getProductsClient().disable(integrationCode, restaurantID);
					Product producto = factory.factoryProduct(item);
					producto.setEnabled(false);
					apiClient.getProductsClient().update(producto, restaurantID.toString());
				} catch (Exception e) {
