...
import cl.ahumada.fuse.descuentos.api.resources.json.PromovarCE;
...
public class GeneraAllRequest implements Processor {
	...
	private CartEntryRequest factoryCartEntry(DescuentosRequest request) {
		...
		if (request != null) {
			...
			prRequest = new CartEntryRequest(store, trxStartTime, trxNumber, factoryPromovarsCE(request.promoVar), factoryItems(request.getProducto()));
			
		}
		return prRequest;
	}
	
	private PromovarCE[] factoryPromovarsCE(Promovar[] promoVar) {
		if ( promoVar == null)
			return null;
		List<PromovarCE> lista = new ArrayList<PromovarCE>();
		for (Promovar prom : promoVar) {
			Long code = prom.getPromovar();
			lista.add(new PromovarCE(code.toString(), null, null));
		}
		return lista.toArray(new PromovarCE[0]);
	}
