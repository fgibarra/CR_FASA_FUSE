	...
	public void process(Exchange exchange) throws Exception {
		...
		if (hayBalanceInquiry()) {
			...
			try {
				...
				descuentosResponse = new DescuentosResponse(transactionCode, producto, response.posMessages, null);
	...
	private DescuentosResponse generarDescuentosConCartEntry() {
		...
		try {
			...
			if (cartEntryResponse != null && cartEntryResponse.items != null) {
				...
				descuentosResponse = new DescuentosResponse(0l, listaProductos.toArray(new Producto[0]), null, null);
			} else {
				descuentosResponse = new DescuentosResponse(1l, null, null, null);
			}
		} catch (Exception e) {
			logger.error("process", e);
		}
